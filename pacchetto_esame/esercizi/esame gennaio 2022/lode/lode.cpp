...
int get_minimum_value(tree* root);
int get_maximum_value(tree* root);
int maxmin_value(tree* root, bool min);

int maxmin_value(tree* root, bool max)
{
    if(root == NULL)
    {
        return -1;
    ...
    if(max)
    {
        return get_maximum_value(root);
    }
    else
    {
        return get_minimum_value(root);
    }
}
